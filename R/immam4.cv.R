...
immam4.cv <- 
  function(Y,X,group,ncv,K_index,r1_index,r2_index,r3_index,r4_index,lambda,D0,intercept,opts,opts_pen){
    n <- opts$n
    q <- opts$q
    p <- opts$p
    G = opts$G
    ...
    RSS = matrix(0,nlam,length(r1_index)*length(r2_index)*length(r3_index)*length(r4_index)*length(K_index))
          ...
          for(r3 in r3_index){
            ...
            for(r2 in r2_index){
              ...
              for(r1 in r1_index){
                ...
                fit = EstPenColumnT4CV(Ytrain,Ztrain,Ytest,Ztest,S,A,B,C,D,lambda,opts,opts_pen) 
